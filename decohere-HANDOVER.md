...
6. **Vincents vraag: "die versie waar de lattice een organische wavey links→rechts flow werd".** ⚠ NIET de groen/paarse activity-sweep (`sweepTone`/node-band) — Vincent bedoelt dat de **LATTICE ZELF (de geometrie) veranderde**, alsof er een soort **honeycomb-algoritme overheen ging** (cellen die zich golvend reorganiseren, cellular-automaton-achtig, L→R). Dit was waarschijnlijk een TRANSIENT in een morph — kandidaten: (a) de `bootstrap`(kubus)→`daemon`(honeycomb-hexagon) morph, waar deeltjes van kubus naar hex-cellen bewegen en de honeycomb "uitkristalliseert"; (b) een moment waar de honeycomb-snap (`lerp(x, cellCentre, 0.66)` in `hexLat`-derivatie) in beweging interacteerde met de morph. **Om te recreëren**: animeer de honeycomb-snap als een BEWEGEND FRONT i.p.v. statisch ingebakken — bijv. `snapAmt = smoothstep(front-w, front, x)` met `front` die L→R schuift (tijd of scroll), deeltjes snappen naar hun hex-celcentrum zodra het front passeert → een honeycomb-patroon dat als een golf over het lattice "groeit"/kristalliseert. Inzetbaar als apart moment op daemon (04) of als overgangseffect. (Snap-logica zit nu in de `hexLat`-bouw in `makeTargets`; voor animatie moet je het naar runtime in `frame()` halen, of een tweede target-set lerpen.)
